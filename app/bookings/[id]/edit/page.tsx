...
export default function EditBooking() {
  ...
  // Mock data – προ-γεμισμένα (real θα γίνει fetch αργότερα)
  const mockBooking = {
    mainService: 'Wine Tasting Tour',
    mainPrice: 450,
    mainIsExternal: false,
    mainExternalPartner: '',
    mainExternalCost: 0,
    extraLegs: [
      { time: '14:30', service: 'Airport Transfer', price: 80, isExternal: false, externalPartner: '', externalCost: 0 },
      { time: '15:30', service: 'Wine Tasting Tour', price: 290, isExternal: false, externalPartner: '', externalCost: 0 },
      { time: '20:00', service: 'Airport Transfer', price: 80, isExternal: false, externalPartner: '', externalCost: 0 }
    ] as ExtraLeg[],
    extraServices: [
      {
        service: 'Meet & Greet',
      ...
      },
      {
        ...
        time: '18:00',
        isExternal: true,
        externalPartner: 'Taxi Athens',
        externalCost: 70,
        pickup: 'Hotel Grande Bretagne',
        dropoff: 'Syntagma Square'
      }
    ] as ExtraService[],
    b2bPartner: 'Grande Bretagne',
    commissionBase: 'gross' as 'gross' | 'net',
    pickup: 'Athens Airport',
  ...
  // States – προ-γεμισμένα από mock
  const [mainService, setMainService] = useState(mockBooking.mainService)
  const [mainPrice, setMainPrice] = useState(mockBooking.mainPrice)
  const [mainIsExternal, setMainIsExternal] = useState(mockBooking.mainIsExternal)
  const [mainExternalPartner, setMainExternalPartner] = useState(mockBooking.mainExternalPartner)
  const [mainExternalCost, setMainExternalCost] = useState(mockBooking.mainExternalCost)
  const [extraLegs, setExtraLegs] = useState<ExtraLeg[]>(mockBooking.extraLegs)
  const [extraServices, setExtraServices] = useState<ExtraService[]>(mockBooking.extraServices)
  const [b2bPartner, setB2bPartner] = useState(mockBooking.b2bPartner)
  const [commissionBase, setCommissionBase] = useState<'gross' | 'net'>(mockBooking.commissionBase)
  const [pickup, setPickup] = useState(mockBooking.pickup)
  const [dropoff, setDropoff] = useState(mockBooking.dropoff)
  const [extraStops, setExtraStops] = useState(mockBooking.extraStops)
  const [date, setDate] = useState(mockBooking.date)
  const [time, setTime] = useState(mockBooking.time)
  const [flight, setFlight] = useState(mockBooking.flight)
  const [reference, setReference] = useState(mockBooking.reference)
  const [leader, setLeader] = useState(mockBooking.leader)
  const [passengers, setPassengers] = useState(mockBooking.passengers)
  const [pax, setPax] = useState<number>(mockBooking.pax)
  const [phone, setPhone] = useState<string>(mockBooking.phone)
  ...
  const handleMainServiceChange = (service: string) => {
    ...
    if (comboLegs[service]) {
      ...
      setMainPrice(servicePrices[service] || 0)
      setExtraLegs([])
    }
  }
  ...
  const handleSaveBooking = () => {
    console.log('Κράτηση ενημερώθηκε!', {
      id,
      mainService,
      mainPrice,
      mainIsExternal,
    ...
    alert(`Κράτηση #${id} ενημερώθηκε επιτυχώς! (mock)`)
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-7xl mx-auto space-y-12">
        <h1 className="text-3xl font-bold text-slate-900">Επεξεργασία Κράτησης #{id}</h1>
        ...
        {/* Λίστα Επιβατών - ΤΕΛΙΚΗ ΔΙΟΡΘΩΜΕΝΗ */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">Λίστα Επιβατών</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Reference Number</label>
              <input type="text" value={reference} onChange={(e) => setReference(e.target.value)} className="w-full px-4 py-3 border border-slate-300 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Passenger's Leader</label>
              <input type="text" value={leader} onChange={(e) => setLeader(e.target.value)} className="w-full px-4 py-3 border border-slate-300 rounded-lg" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input 
                type="email" 
                value={phone.includes('@') ? phone : ''} // Προαιρετικό: αν θες να προ-γεμίζει από phone
                onChange={(e) => setPhone(e.target.value)} // Μπορείς να προσθέσεις ξεχωριστό state για email
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <input type="datetime-local" value={`${date}T${time}`} onChange={e => {
            ...
            <div>
              ...
              <input type="text" value={flight} onChange={(e) => setFlight(e.target.value)} className="w-full px-4 py-3 border border-slate-300 rounded-lg" placeholder="A3 123" />
            </div>
          </div>
          <div className="mb-6">
          ...
          {/* Οικονομικά - ΔΙΟΡΘΩΜΕΝΗ ΕΜΦΑΝΙΣΗ */}
