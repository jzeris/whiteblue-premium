...
import { useState, useRef } from 'react' // ← useRef για interval
...
export default function JobDetails() {
  ...
  // Mock data – αργότερα από Supabase / localStorage
  ...
  // ΝΕΟ: Λειτουργία Collect Payment
  const handleCollectPayment = () => {
    if (status !== 'en_route' && status !== 'completed') {
      alert('Η δουλειά πρέπει να έχει ξεκινήσει πρώτα!')
      return
    }

    setStatus('completed')
    alert('Πληρωμή εισπράχθηκε! Η κράτηση ολοκληρώθηκε.')
    
    // Προαιρετικά: update localStorage αργότερα
    // const bookings = JSON.parse(localStorage.getItem('bookings') || '[]')
    // const updated = bookings.map(b => b.id === jobId ? { ...b, status: 'completed' } : b)
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24">
      ...
      <main className="flex-1 p-4 space-y-6">
        ...
        <section className="bg-white rounded-xl p-5 shadow-sm border border-gray-200">
          ...
          <button 
            onClick={handleCollectPayment} // ← ΝΕΟ: συνδέεται με τη συνάρτηση
            disabled={status !== 'en_route' && status !== 'completed'}
